...
export function RecetaMaestraList() {
  ...
  const [modalEliminar, setModalEliminar] = useState<RecetaMaestra | null>(null)
  const [motivo, setMotivo] = useState('')
  const [form, setForm] = useState({ codigo: '', descripcion: '', version: '' })
      ...
      render: (r) => {
        ...
        return (
          <div className="dt-act">
            <button className="dt-ab dt-ab-edit" title="Editar datos básicos" onClick={() => { setEditando(r); setForm({ codigo: r.codigo, descripcion: r.descripcion, version: r.version }); setModalCrear(true) }}><i className="fa fa-pencil-alt" /></button>
            ...
            <button className="dt-ab dt-ab-del" title="Eliminar" onClick={() => setModalEliminar(r)}><i className="fa fa-trash-alt" /></button>
            {sig && (
    ...
    <>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
        ...
        <div style={{ paddingTop: 4 }}>
          <button className="btn btn-success" onClick={() => setModalCrear(true)}>
        ...
        <div className="modal-overlay" onClick={() => { setModalCrear(false); setEditando(null) }}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-footer-bar">
              ...
              <button className="btn btn-primary" onClick={() => { setModalCrear(false); setEditando(null) }}><i className="fa fa-check" /> {editando ? 'Guardar' : 'Crear'}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={() => setModalEliminar(null)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div className="modal-header-bar" style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              ...
              <button style={{ background:'none', border:'none', cursor:'pointer', color:'var(--ink-4)', fontSize:18 }} onClick={() => setModalEliminar(null)}>×</button>
            </div>
            <div className="modal-body-area" style={{ fontSize:14, color:'var(--ink-2)', lineHeight:1.6 }}>
              ¿Está seguro que desea eliminar la receta <strong>{modalEliminar.codigo}</strong> — {modalEliminar.descripcion}? Esta acción no se puede deshacer.
            </div>
            <div className="modal-footer-bar">
              <button className="btn btn-gray" onClick={() => setModalEliminar(null)}><i className="fa fa-undo" /> Cancelar</button>
              <button className="btn btn-danger" onClick={() => setModalEliminar(null)}><i className="fa fa-trash-alt" /> Eliminar</button>
            </div>
          </div>
        </div>
      )}
